  ...
  return (
    <main className={cn("min-h-screen p-4 md:p-6", mode === "tv" && "p-7")}>
      <div className="flex w-full flex-col gap-4">
        ...
        <div className={cn("grid gap-3", mode === "tv" ? "grid-cols-6" : "grid-cols-2 lg:grid-cols-6")}>
          {BUSINESS_STATUSES.map((_, index) => (
            <Skeleton key={index} className="h-24" />
          ))}
        </div>
        <div className={cn("grid gap-3", mode === "tv" ? "grid-cols-6" : "grid-cols-1 xl:grid-cols-6")}>
          {BUSINESS_STATUSES.map((_, column) => (
