    ...
    <div className="min-w-0 rounded-lg border bg-card text-card-foreground shadow-sm">
      <div className={cn("border-b", mode === "tv" ? "p-2.5" : "p-3")}>
        <div className="flex items-center justify-between gap-2">
          ...
          <span className={cn("rounded-md bg-muted px-2 py-1 font-semibold", mode === "tv" ? "text-sm" : "text-xs")}>
      ...
      <div className={cn("flex flex-col gap-2 p-2.5", mode === "tv" ? "max-h-[72vh] overflow-hidden" : "min-h-40")}>
        {issues.length ? (
          issues.map((issue) => (
            <IssueCard
              key={issue.id}
